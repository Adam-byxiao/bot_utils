...
class ROISelectorDialog(wx.Dialog):
    def __init__(self, parent, image_path):
        super().__init__(parent, title="选择24色卡区域", size=(800, 600))
    ...
    def _init_coord_transform(self):
        """初始化坐标转换参数"""
        self.renderer = self.canvas.renderer
        self.bbox = self.ax.get_window_extent().transformed(
            self.figure.dpi_scale_trans.inverted())
        self.ax_width = self.bbox.width * self.figure.dpi
        self.ax_height = self.bbox.height * self.figure.dpi
    ...
    def _convert_coords(self, x, y):
        """将窗口坐标转换为图像坐标"""
        # 转换为相对于axes的坐标
        x_ax = (x - self.bbox.x0 * self.figure.dpi) / self.ax_width * self.width
        y_ax = (y - self.bbox.y0 * self.figure.dpi) / self.ax_height * self.height
        
        # 转换为图像坐标并限制范围
        img_x = max(0, min(int(x_ax), self.width-1))
        img_y = max(0, min(int(y_ax), self.height-1))
