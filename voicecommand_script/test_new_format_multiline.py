...
def test_new_log_format_multiline():
    """测试新的多行日志格式解析"""
    parser = KWSLogParser()
    ...
    test_logs = [
        ...
        # 新的结果详情行（多行合并为单行）
        "2026-02-06T06:10:40.982660Z INFO vibe-ai-server: [kws_sensory.cc(914)] --- Model:hey_vibe Results --- Raw Logits:        [Label 0: 6.53906, Label 1: -6.87891] Probabilities:     [Label 0: 0.999999, Label 1: 1.48816e-06] ------------------------- ==> Predicted Label ID: 0 ==> Confidence Score:   0.999999 ==> Preprocess Cost:    11ms ==> Rknn Run Cost:      211ms ==> Sum Cost:     223ms",
        
        # 另一个结果详情行 (Label ID为1)
        "2026-02-06T06:11:15.123456Z INFO vibe-ai-server: [kws_sensory.cc(914)] --- Model:hello_vibe Results --- Raw Logits:        [Label 0: -2.34567, Label 1: 8.91234] Probabilities:     [Label 0: 0.000123, Label 1: 0.999877] ------------------------- ==> Predicted Label ID: 1 ==> Confidence Score:   0.999877 ==> Preprocess Cost:    15ms ==> Rknn Run Cost:      198ms ==> Sum Cost:     213ms"
    ]
    
    print("🧪 测试新的KWS多行日志格式解析")
    print("="*60)
    ...
    for i, log in enumerate(test_logs):
        ...
        # 尝试解析KWS识别行
        kws_data = parser.parse_kws_line(log)
        if kws_data:
            print("   ✅ KWS识别行解析成功:")
        ...
        # 尝试解析结果详情行
        results_data = parser.parse_results_line(log)
        if results_data:
            print("   ✅ 结果详情行解析成功:")
